...
version: "0.2.1"
author: myagentzero
description: "Deep web research with source citation. Summarizes findings accurately."
tags: ["web research", "information synthesis", "source citation", "fact-checking"]
---
...
2. **Search broadly, then narrow.** Start with 2-3 diverse search queries to cover different angles of the topic. Use `web_search_tool` to find candidate sources. Refine queries based on initial results.
3. **Read primary sources.** Use `browser` to read the full content of promising pages. Prioritize:
...
| Tool | Usage |
|------|-------|
| `web_search_tool` | Search the web by query string. Use specific, targeted queries. |
| `browser` | Get and read the full content of a URL. Use this to verify claims before citing. |
