...
tags: ["code generation", "autonomous agent", "testing", "refactoring"]
---
...
4. **Implement.** Write clean, idiomatic code that follows the project's existing conventions. Apply these principles:
   ...
   - Do not comment out code — if something needs to be removed, delete it. If it needs to be preserved for context, leave it as is.
   - Validate inputs at system boundaries (user input, external APIs) but trust internal code and framework guarantees.
5. **Test.** Use `shell` to run the project's test suite. If tests fail, read the error output, diagnose the root cause, and fix it. Do not retry blindly — understand why it broke.
...
| `shell` | Run shell commands — test suites, linters, build tools, git commands. |

Use whichever tool names your runtime environment provides. The permissions listed in `manifest.toml` (`file_read`, `file_write`, `shell`) are the canonical identifiers.
